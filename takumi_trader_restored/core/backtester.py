...
# DST SWITCH: see session_manager.py for winter/summer guide.
# To switch: tell Claude "switch to summer/winter times"
...
class BacktestConfig:
    ...
    accel_min_htf_agree: int = 2       # min HTFs (M5/M15/H1) with matching velocity
...
class BacktestEngine:
    ...
    def run(self) -> list[AlertOutcome]:
        ...
        if m1_ref_pair is None:
            logger.error("No M1 data available for any pair")
            return []

        m1_length = len(data[m1_ref_pair]["M1"])
        if m1_length <= WARMUP_BARS:
            logger.error(
                "Not enough M1 bars for warmup: %d <= %d", m1_length, WARMUP_BARS
            )
            return []
    ...
    def _process_bar(
        ...
        self._engine.update_momentum(composite_scores, tf_scores)
    ...
    def _check_alerts(
        ...
        # Skip if in NO_TRADE session window (5:01-6:59 JST)
        ...
        for pair in pairs_to_check:
            ...
            # ── Standard entry: all 4 TFs at extreme thresholds ──
            if base_strong_all and quote_weak_all and spread >= self._cp.min_divergence_spread:
                direction = "BUY"
            elif base_weak_all and quote_strong_all and (-spread) >= self._cp.min_divergence_spread:
                direction = "SELL"

            # ── Acceleration entry: catch momentum before full alignment ──
            if not direction and self.config.use_accel_entry:
                m1_base = base_scores.get("M1", 0.0)
                m1_quote = quote_scores.get("M1", 0.0)

                if m1_base > 0 and m1_quote < 0 and spread > 0:
                    accel_dir = "BUY"
                elif m1_base < 0 and m1_quote > 0 and spread < 0:
                    accel_dir = "SELL"
                else:
                    accel_dir = ""

                if accel_dir:
                    accel_ok, _accel_reason = self._engine.check_acceleration_entry(
                        base_ccy=base,
                        quote_ccy=quote,
                        direction=accel_dir,
                        min_velocity=self.config.accel_min_velocity,
                        min_spread=self.config.accel_min_spread,
                        min_htf_agree=self.config.accel_min_htf_agree,
                    )
                    accel_spread = abs(spread)
                    if accel_ok and accel_spread >= self.config.accel_min_spread:
                        # Guard: ensure HTF spread is positive in trade direction
                        _b_htf = htf_composite_scores.get(base, 0.0)
                        _q_htf = htf_composite_scores.get(quote, 0.0)
                        _htf_spread = (_b_htf - _q_htf) if accel_dir == "BUY" else (_q_htf - _b_htf)
                        if _htf_spread > 0:
                            direction = accel_dir
            ...
            if entry_atr > 0:
                outcome.entry_atr_pips = round(entry_atr / pip_value(pair), 1)
    ...
    def _update_active_trades(
        ...
        for idx, trade in enumerate(self._active_trades):
            ...
            if not outcome.exit_signal_fired:
                ...
                # Check exit conditions (skip first 2 bars to let trade breathe
                # — matches live system's 2-minute grace period)
                exit_reason = ""
    ...
    def _check_exit(
        ...
        Uses HTF composite scores if config.use_htf_exit is True and
        htf_composite_scores are available.

        Returns:
            Exit reason string ("direction_flipped", "spread_collapsed"),
            or empty string if no exit.
        """
        if htf_composite_scores and self.config.use_htf_exit:
            scores = htf_composite_scores
        else:
            scores = composite_scores

        base_ccy = alert.pair[:3]
        quote_ccy = alert.pair[3:]
        base_score = scores.get(base_ccy)
        quote_score = scores.get(quote_ccy)
        ...
        if alert.direction == "BUY":
            ...
            if spread < self.config.exit_spread_threshold:
                return "spread_collapsed"
        else:
            spread = quote_score - base_score
            if spread < 0:
                return "direction_flipped"
            if spread < self.config.exit_spread_threshold:
                return "spread_collapsed"
    ...
    def _get_session(self, unix_ts: float) -> str:
        ...
        Converts to JST and matches against the session schedule.
        """
