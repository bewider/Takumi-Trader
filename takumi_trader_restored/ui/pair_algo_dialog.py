...
class PairAlgoDialog(QDialog):
    ...
    def _build_settings_html(self, pair: str, settings: dict, history_count: int) -> str:
        ...
        parts.append('<h3>Calculation Parameters</h3>')
        parts.append('<table>')
        parts.append('<tr><th>Parameter</th><th>Value</th><th>Default</th></tr>')

        _defaults = {
            "ema_period": 8, "roc_decay": 0.3, "sensitivity": 1.0,
            "threshold_m1": 6.5, "min_divergence_spread": 12.0,
            "require_acceleration": False,
        }

        for param, default in _defaults.items():
            val = settings.get(param, default)
            changed = val != default
            val_class = "param-value" if changed else "stat-neutral"
            change_marker = " *" if changed else ""
            parts.append(
                f'<tr>'
                f'<td class="param-name">{param}</td>'
                f'<td class="{val_class}">{val}{change_marker}</td>'
                f'<td class="stat-neutral">{default}</td>'
                f'</tr>'
            )
        parts.append('</table>')
...
class PairAlgoHistoryDialog(QDialog):
    ...
    def _entry_html(self, entry: dict, is_current: bool = False) -> str:
        ...
        # Params
        _params = ["ema_period", "roc_decay", "sensitivity", "threshold_m1",
                    "min_divergence_spread", "require_acceleration"]
        param_strs = []
        for p in _params:
            if p in entry:
                param_strs.append(f"{p}=<b>{entry[p]}</b>")
        if param_strs:
            lines.append(f'<br>Params: {" &nbsp;|&nbsp; ".join(param_strs)}')
