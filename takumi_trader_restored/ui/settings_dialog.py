...
def load_settings() -> dict:
    ...
    return {
        ...
        # MT5 auto-trading
        "mt5_trading_enabled": s.value("mt5/enabled", False, type=bool),
        "mt5_auto_open": s.value("mt5/auto_open", True, type=bool),
        "mt5_auto_close": s.value("mt5/auto_close", True, type=bool),
        "mt5_risk_pct": s.value("mt5/risk_pct", 1.0, type=float),
        "mt5_max_positions": s.value("mt5/max_positions", 5, type=int),
    }
...
def save_settings(settings: dict) -> None:
    ...
    # MT5 auto-trading
    s.setValue("mt5/enabled", settings.get("mt5_trading_enabled", False))
    s.setValue("mt5/auto_open", settings.get("mt5_auto_open", True))
    s.setValue("mt5/auto_close", settings.get("mt5_auto_close", True))
    s.setValue("mt5/risk_pct", settings.get("mt5_risk_pct", 1.0))
    s.setValue("mt5/max_positions", settings.get("mt5_max_positions", 5))
...
class SettingsDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        # ── MT5 Auto-Trading ──────────────────────────────────────
        mt5_group = QGroupBox("MT5 Auto-Trading (IC Markets Demo)")
        mt5_layout = QVBoxLayout(mt5_group)
        mt5_layout.setSpacing(6)

        self.chk_mt5_enabled = QCheckBox("Enable MT5 auto-trading")
        mt5_layout.addWidget(self.chk_mt5_enabled)

        # Risk % + max positions
        mt5_param_row = QHBoxLayout()
        mt5_param_row.addWidget(QLabel("Risk per trade:"))
        self.spin_mt5_risk = QDoubleSpinBox()
        self.spin_mt5_risk.setRange(0.1, 10.0)
        self.spin_mt5_risk.setSingleStep(0.1)
        self.spin_mt5_risk.setDecimals(1)
        self.spin_mt5_risk.setSuffix("%")
        self.spin_mt5_risk.setValue(1.0)
        mt5_param_row.addWidget(self.spin_mt5_risk)
        mt5_param_row.addSpacing(16)
        mt5_param_row.addWidget(QLabel("Max positions:"))
        self.spin_mt5_max_pos = QSpinBox()
        self.spin_mt5_max_pos.setRange(1, 27)
        self.spin_mt5_max_pos.setValue(5)
        mt5_param_row.addWidget(self.spin_mt5_max_pos)
        mt5_param_row.addStretch()
        mt5_layout.addLayout(mt5_param_row)

        # Auto-open / auto-close
        self.chk_mt5_auto_open = QCheckBox("Auto-open orders on FULL conviction alerts")
        self.chk_mt5_auto_close = QCheckBox("Auto-close on spread-collapse signal exit")
        self.chk_mt5_auto_open.setChecked(True)
        self.chk_mt5_auto_close.setChecked(True)
        mt5_layout.addWidget(self.chk_mt5_auto_open)
        mt5_layout.addWidget(self.chk_mt5_auto_close)

        mt5_note = QLabel(
            "⚠ Requires: MT5 → Tools → Options → Expert Advisors → ✓ Allow algorithmic trading\n"
            "Position size auto-calculated from risk % and SL distance. SL/TP set directly on broker order."
        )
        mt5_note.setStyleSheet("color: #888; font-size: 9pt;")
        mt5_note.setWordWrap(True)
        mt5_layout.addWidget(mt5_note)
    ...
    def _load_current(self) -> None:
        ...
        # MT5
        self.chk_mt5_enabled.setChecked(settings.get("mt5_trading_enabled", False))
        self.chk_mt5_auto_open.setChecked(settings.get("mt5_auto_open", True))
    ...
    def get_settings(self) -> dict:
        ...
        return {
            ...
            # MT5
            "mt5_trading_enabled": self.chk_mt5_enabled.isChecked(),
            "mt5_auto_open": self.chk_mt5_auto_open.isChecked(),
