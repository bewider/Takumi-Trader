...
class HTFRegimeTracker:
    ...
    # BUG #5 FIX (2026-04-21): hysteresis band to prevent regime flicker at
    # threshold boundaries. Without hysteresis, strength oscillating between
    # 2.99 and 3.01 (with default threshold=3.0) would flip the regime
    # BULLISH/NEUTRAL on every H4 close. With this band:
    #   Enter new regime:  |strength| must exceed threshold + _HYSTERESIS_MARGIN
    #   Exit current regime: |strength| must drop below threshold - _HYSTERESIS_MARGIN
    # So a BULLISH currency at strength=2.9 stays BULLISH (still above 3.0 - 0.5
    # = 2.5); drops to NEUTRAL only when strength falls below 2.5. Smooths the
    # transition without being sluggish.
    _HYSTERESIS_MARGIN = 0.5
    ...
    def _update_regimes(self, tf: str) -> None:
        ...
        for ccy in CURRENCIES:
            ...
            # Hysteresis: current regime "sticks" until strength moves far
            # enough past the threshold in the OTHER direction.
            prev_regime, _prev_strength = self._regimes[ccy].get(tf, ("NEUTRAL", 0.0))
            entry_high = self.threshold + self._HYSTERESIS_MARGIN
            exit_high = self.threshold - self._HYSTERESIS_MARGIN
            entry_low = -(self.threshold + self._HYSTERESIS_MARGIN)
            exit_low = -(self.threshold - self._HYSTERESIS_MARGIN)

            if prev_regime == "BULLISH":
                # Stay BULLISH unless strength drops below exit_high
                if strength < exit_high:
                    regime = "BEARISH" if strength < entry_low else "NEUTRAL"
                else:
                    regime = "BULLISH"
            elif prev_regime == "BEARISH":
                # Stay BEARISH unless strength rises above exit_low
                if strength > exit_low:
                    regime = "BULLISH" if strength > entry_high else "NEUTRAL"
                else:
                    regime = "BEARISH"
            else:  # NEUTRAL — require FULL threshold+margin to enter a regime
                if strength > entry_high:
                    regime = "BULLISH"
                elif strength < entry_low:
                    regime = "BEARISH"
                else:
                    regime = "NEUTRAL"
