...
def speak(text: str) -> None:
    """Speak text aloud using Microsoft Edge neural TTS in a background thread.

    Uses the natural-sounding 'en-US-GuyNeural' voice via edge-tts.
    Falls back to pyttsx3 (robotic) if edge-tts is unavailable.
    """
    def _speak() -> None:
        try:
            import asyncio
            ...
            async def _gen():
                tmp = os.path.join(tempfile.gettempdir(), "takumi_tts.mp3")
                comm = edge_tts.Communicate(text, "en-US-AriaNeural", rate="+5%")
                await comm.save(tmp)
                play_sound(tmp)
...
class AlertManager:
    ...
    def check_and_fire(
        self,
        current_alert_pairs: dict[str, tuple[str, dict[str, float]]],
        ...
        Args:
            ...
            open_pairs: Set of pairs with open trades. Sound is suppressed
                for these pairs (alert still shows in panel).
        ...
        for pair in newly_entered:
            ...
            score_parts = " | ".join(
                f"{tf}: {scores[tf]:+.1f}" for tf in ["M5", "M15", "H1"]
                if tf in scores
            )
            body = f"{pair} — {direction} ({score_parts})"

            # Toast popup disabled — user request (too noisy, fires even for
            # blocked/non-traded signals). Re-enable by uncommenting:
            # send_toast_notification("TAKUMI Trader", body)

            # Sound (suppressed if trade already open on this pair)
            if self.sound_enabled and self.sound_file:
                if not (open_pairs and pair in open_pairs):
