...
# Stoch v2: entries handled by StochStrengthEngine — no grid search needed
# Single fixed combo so optimizer only varies SL/TP (the only thing that matters)
CALC_PARAM_GRID = {
    "ema_period":       [4],
    "roc_decay":        [0.2],
    "sensitivity":      [1.0],
...
CALC_PARAM_GRID_FAST = dict(CALC_PARAM_GRID)
CALC_PARAM_GRID_TURBO = dict(CALC_PARAM_GRID)
...
def optimize_calc_params(
    ...
    # Sort by: prioritize WR >= 80%, then best total R within that pool.
    # Falls back to lower WR thresholds if no combos meet 80%.
    for wr_threshold in [80.0, 75.0, 70.0, 0.0]:
        good_results = [r for r in results if r.wr >= wr_threshold]
        if good_results:
            good_results.sort(key=lambda x: (x.wr, x.total_r), reverse=True)
            bad_results = [r for r in results if r.wr < wr_threshold]
            bad_results.sort(key=lambda x: x.total_r, reverse=True)
            results = good_results + bad_results
            break
