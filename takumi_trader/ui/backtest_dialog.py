...
class BacktestWorker(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            n = len(results)
            logger.info("Backtest produced %d trades", n)

            # Run SL/TP optimization on the worker thread (heavy computation)
            # Done BEFORE stripping bar data since optimizer needs it
            all_combos = []
            profile = {}
            atr_combos = []
            atr_profile = {}
            if n > 0:
                try:
                    self.status.emit(f"Optimizing fixed-pip SL/TP ({n} trades)...")
                    all_combos = _optimize_sl_tp(results)
                except Exception as exc:
                    logger.error("Fixed-pip SL/TP optimization failed: %s", exc)

                try:
                    self.status.emit(f"Building SL/TP profiles ({n} trades)...")
                    profile = _build_sltp_profile(results)
                except Exception as exc:
                    logger.error("Fixed-pip profiles failed: %s", exc)

                atr_count = sum(1 for r in results if r.entry_atr_pips > 0)
                if atr_count > 0:
                    try:
                        self.status.emit(f"Optimizing ATR SL/TP ({atr_count} trades)...")
                        atr_combos = _optimize_atr_sl_tp(results)
                    except Exception as exc:
                        logger.error("ATR SL/TP optimization failed: %s", exc)

                    try:
                        self.status.emit(f"Building ATR profiles ({atr_count} trades)...")
                        atr_profile = _build_atr_sltp_profile(results)
                    except Exception as exc:
                        logger.error("ATR profiles failed: %s", exc)

            # Save results to temp file instead of passing through Qt signal
            # (large payloads crash Qt's cross-thread serialization)
            ...
            tmp = Path(tempfile.gettempdir()) / "takumi_backtest_result.json"
            tmp.write_text(json.dumps(payload, ensure_ascii=False), encoding="utf-8")

            del results, outcome_dicts, payload
            gc.collect()

            # Signal carries only the file path (tiny payload)
            self.finished.emit({"_result_file": str(tmp)})
        except Exception as e:
            import traceback
...
def _find_best_atr_sltp(
    outcomes: list, min_wr: float = 80.0, min_sl_pips: float = 5.0,
) -> dict | None:
    """Find best ATR-based SL/TP for a group: WR >= min_wr (80%), best expectancy in R."""
    arrays = _precompute_atr_arrays(outcomes)
    if arrays is None:
        return None
    ...
    for sl_m in sl_range:
        for tp_m in tp_range:
            # Enforce TP >= SL (never risk more than you can gain)
            if tp_m < sl_m:
                continue
            c = _eval_atr_sl_tp_fast(sl_m, tp_m, *arrays, min_sl_pips=min_sl_pips)
            if c is not None:
    ...
    for threshold in [min_wr, 75.0, 65.0, 50.0]:
        pool = [c for c in candidates if c["wr"] >= threshold and c["total_pnl"] > 0]
        if pool:
            # Prioritize WR first, then total R as tiebreaker
            pool.sort(key=lambda x: (x["wr"], x["total_r"]), reverse=True)
            return pool[0]
...
class BacktestDialog(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self._chk_news.setChecked(True)
        ...
        self._chk_accel.setChecked(False)
    ...
    def _on_finished(self, payload: dict) -> None:
        try:
            # Load results from temp file if worker used file-based transfer
            if "_result_file" in payload:
                import json as _json
                _path = Path(payload["_result_file"])
    ...
    def _on_error(self, msg: str) -> None:
        ...
        if "Cancelled" in msg:
            ...
            # Show a popup so the user doesn't miss the error
            from PyQt6.QtWidgets import QMessageBox
            short_msg = msg.split("\n")[0][:200]
            QMessageBox.warning(self, "Backtest Error", short_msg)
    ...
    def _save_trade_log_csv(self, pair: str, best) -> None:
        ...
        for trade_no, o in enumerate(outcomes, 1):
            ...
            entry_dt = _dt.fromtimestamp(o.entry_time, tz=_JST)
            pip = pip_value(pair)
            ...
            rows.append({
                ...
                "time_jst": entry_dt.strftime("%H:%M"),
